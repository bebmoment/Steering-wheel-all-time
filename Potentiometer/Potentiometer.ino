...
int offset = 436;
float EMA_accel_a = 0.6;      //initialization of accelerator EMA alpha
float EMA_brake_a = 0.6;
float EMA_clutch_a = 0.6;
...
void loop(){
  ...
  // EMA filtering
  if(EMA){
    EMA_ACCELERATOR = (EMA_accel_a*acceleratorValue) + ((1-EMA_accel_a)*EMA_ACCELERATOR);    //run accelerator EMA
    EMA_BRAKE = (EMA_brake_a*brakeValue) + ((1-EMA_brake_a)*EMA_BRAKE);    //run brake EMA
    EMA_CLUTCH = (EMA_clutch_a*clutchValue) + ((1-EMA_clutch_a)*EMA_CLUTCH);    //run brake EMA

    acceleratorCommanded = EMA_ACCELERATOR + offset;
    brakeCommanded = EMA_BRAKE + offset;
    clutchCommanded = EMA_CLUTCH + offset;
  
    Serial.print(acceleratorCommanded); //print digital value to serial... add 512 as map input range is 512-1023
    Serial.print(", ");
    ...
    acceleratorCommanded = acceleratorValue + offset;
    brakeCommanded = brakeValue + offset;
    clutchCommanded = clutchValue + offset;
